...
const Sidebar = () => {
  ...
  return (
    ...
    >
      {/* Logo Area */}
      <div
        className={`flex h-16 items-center border-b border-gray-100 dark:border-neutral-800 transition-all duration-300 ${isCollapsed ? 'justify-center px-0' : 'px-6 gap-3'}`}
      >
        ...
        <div
          className={`flex flex-col overflow-hidden whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0' : 'w-auto opacity-100'}`}
        >
          <h1 className="text-gray-900 dark:text-white text-sm font-bold leading-none tracking-tight">
            Procura
          </h1>
          <p className="text-gray-500 text-[10px] mt-1 font-medium tracking-wide uppercase">
            Ops Command
          </p>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-1 p-3 overflow-y-auto flex-1 overflow-x-hidden custom-scrollbar">
        {!isCollapsed && (
          <p className="px-3 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest animate-in fade-in duration-300">
            Main
          </p>
        )}
        ...
        <NavLink to="/dashboard" className={navItemClass} title="Browse and qualify opportunities">
          ...
          <span
            className={`text-sm font-medium whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100 block'}`}
          >
            Dashboard
          </span>
        </NavLink>
        ...
        <NavLink
          ...
          title="Pipeline — track opportunities by stage"
        >
        ...
        <NavLink
          to="/market-intel"
          className={navItemClass}
          title="Federal market intelligence from USAspending.gov"
        >
        ...
        {!isCollapsed && (
          <div className="my-2 border-t border-gray-100 dark:border-neutral-800 mx-3"></div>
        )}
        {!isCollapsed && (
          <p className="px-3 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-widest animate-in fade-in duration-300">
            Tracking
        ...
        <NavLink
          to="/correspondence"
          className={navItemClass}
          title="Awards, communications & notifications"
        >
        ...
        <NavLink
          to="/company-profile"
          className={navItemClass}
          title="Your company profile for AI scoring"
        >
      ...
      <div ref={notifRef} className="relative px-3 pb-1">
        ...
        {showNotifDropdown && (
          <div
            className={`absolute bottom-12 ${isCollapsed ? 'left-16' : 'left-3 right-3'} z-50 bg-white border border-gray-200 rounded-xl shadow-xl overflow-hidden min-w-[300px]`}
          >
            ...
            <div className="max-h-80 overflow-y-auto divide-y divide-gray-50">
              {notifications.length === 0 ? (
                <div className="px-4 py-6 text-center text-xs text-gray-400">
                  ...
                  <br />
                  High-fit opportunities will appear here.
                </div>
              ) : (
                notifications.map((n) => (
                  <button
                  ...
                  >
                    <div className="flex items-start gap-2">
                      <span
                        className={`mt-1.5 h-2 w-2 rounded-full shrink-0 ${!n.read ? priorityColor(n.priority) : 'bg-gray-200'}`}
                      />
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-xs font-medium truncate ${!n.read ? 'text-gray-900' : 'text-gray-600'}`}
                        >
                          {n.title}
                        </p>
                      ...
                      {n.action_url && (
                        <ExternalLink size={10} className="shrink-0 mt-1.5 text-gray-300" />
                      )}
                    </div>
                  </button>
                ))
      ...
      <div className="p-3 border-t border-gray-200 dark:border-neutral-800 flex flex-col gap-2">
        <div
          className={`flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors cursor-pointer ${isCollapsed ? 'justify-center' : ''}`}
        >
          <div className="h-8 w-8 rounded-full bg-gradient-to-br from-indigo-500 to-purple-500 flex items-center justify-center text-white text-xs font-bold shrink-0 shadow-sm ring-2 ring-white dark:ring-neutral-900">
            {initials}
          </div>
          <div
            className={`flex flex-col overflow-hidden whitespace-nowrap transition-all duration-300 ${isCollapsed ? 'w-0 opacity-0 hidden' : 'w-auto opacity-100 block'}`}
          >
