...
interface APIResponse<T> {
  ...
  error?: string;
  status: number;
  /** Present when backend returns { data, total } for list endpoints */
  total?: number;
  /** Present when backend returns { data, checks } for get-follow-up */
  checks?: unknown[];
}
...
class ProcuraAPI {
  ...
  private async request<T>(
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (typeof parsed?.total === 'number') out.total = parsed.total;
        if (Array.isArray(parsed?.checks)) out.checks = parsed.checks;
        return out;
      } catch (error) {
        ...
        return {
          error:
            error instanceof Error ? `${error.message} (${method} ${endpoint})` : 'Unknown error',
          status: 0,
        };
      }
    }
  ...
  async getOpportunities(params?: {
    ...
    if (params?.min_fit_score !== undefined)
  ...
  async getAuditLogs(params?: { submission_id?: string; portal?: string; limit?: number }) {
    const query = new URLSearchParams();
    if (params?.limit) query.set('limit', params.limit.toString());
  ...
  async rejectSubmission(id: string, reason: string) {
    return this.request<any>(
      'POST',
      `/submissions/${id}/reject?reason=${encodeURIComponent(reason)}`
    );
  }
  ...
  async updateSubmissionTask(submissionId: string, taskId: string, completed: boolean) {
    return this.request<any>(
      'PATCH',
      `/submissions/${submissionId}/tasks/${taskId}?completed=${completed}`
    );
  }
  ...
  async createFollowUp(submissionId: string, checkIntervalHours: number = 24) {
    return this.request<any>(
      'POST',
      `/follow-ups?submission_id=${submissionId}&check_interval_hours=${checkIntervalHours}`
    );
  }

  async updateFollowUp(
    id: string,
    data: { status_update?: string; auto_check?: boolean; check_interval_hours?: number }
  ) {
    const query = new URLSearchParams();
    Object.entries(data).forEach(([k, v]) => {
      if (v !== undefined) query.set(k, String(v));
  ...
  async listCorrespondence(params?: {
    type?: string;
    status?: string;
  ...
  async listNotifications(unreadOnly: boolean = false, page: number = 1) {
    return this.request<any>(
      'GET',
      `/correspondence/notifications/list?unread_only=${unreadOnly}&page=${page}`
    );
  }
  ...
  async generateFullProposal(submissionId: string, sections?: string[], provider?: string) {
    return this.request<{
      ...
      status: string;
    }>('POST', `/submissions/${submissionId}/generate-proposal`, { sections, provider });
  }
  ...
  async getMarketSummary(naics_codes: string) {
    return this.request<any>(
      'GET',
      `/market-intel/summary?naics_codes=${encodeURIComponent(naics_codes)}`
    );
  }
  ...
  async getIncumbents(naics_codes: string, limit: number = 15) {
    return this.request<any>(
      'GET',
      `/market-intel/incumbents?naics_codes=${encodeURIComponent(naics_codes)}&limit=${limit}`
    );
  }
  ...
  async getAgencyTrend(name: string, years: number = 3) {
    return this.request<any>(
      'GET',
      `/market-intel/agency?name=${encodeURIComponent(name)}&years=${years}`
    );
  }
}
