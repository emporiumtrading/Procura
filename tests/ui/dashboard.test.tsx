  ...
  test('filters work correctly with various combinations', async ({ page }) => {
    ...
    await page.route('**/api/opportunities**', route => {
      route.fulfill({
        ...
        json: {
          data: [
            { id: 'opp-1', title: 'Cybersecurity Opportunity', agency: 'DOD', source: 'sam_gov', fit_score: 95, status: 'qualified' },
            { id: 'opp-2', title: 'IT Modernization', agency: 'DHS', source: 'govcon', fit_score: 85, status: 'new' },
            { id: 'opp-3', title: 'Cloud Services', agency: 'NASA', source: 'sam_gov', fit_score: 75, status: 'qualified' },
            { id: 'opp-4', title: 'Network Infrastructure', agency: 'DoD', source: 'sam_gov', fit_score: 65, status: 'new' }
          ],
          total: 4,
          page: 1,
          limit: 25
  ...
  test('handles rapid filter toggling without errors', async ({ page }) => {
    ...
    await page.route('**/api/opportunities**', route => {
      route.fulfill({
        ...
        json: {
          data: [
            { id: 'opp-1', title: 'Opportunity 1', agency: 'Agency 1', source: 'sam_gov', fit_score: 90, status: 'new' },
            { id: 'opp-2', title: 'Opportunity 2', agency: 'Agency 2', source: 'govcon', fit_score: 80, status: 'qualified' },
            { id: 'opp-3', title: 'Opportunity 3', agency: 'Agency 3', source: 'sam_gov', fit_score: 70, status: 'new' }
          ],
          total: 3,
          page: 1,
          limit: 25
  ...
  test('search functionality works with special characters', async ({ page }) => {
    ...
    await page.route('**/api/opportunities**', route => {
      route.fulfill({
        ...
        json: {
          data: [
            { id: 'opp-1', title: 'Cybersecurity Assessment', agency: 'DOD', source: 'sam_gov', fit_score: 95, status: 'qualified' },
            { id: 'opp-2', title: 'IT & Network Services', agency: 'DHS', source: 'govcon', fit_score: 85, status: 'new' },
            { id: 'opp-3', title: 'Cloud (AWS) Services', agency: 'NASA', source: 'sam_gov', fit_score: 75, status: 'qualified' }
          ],
          total: 3,
          page: 1,
          limit: 25
  ...
  test('sort functionality works correctly', async ({ page }) => {
    ...
    await page.route('**/api/opportunities**', route => {
      route.fulfill({
        ...
        json: {
          data: [
            { id: 'opp-1', title: 'Opportunity A', agency: 'Agency A', source: 'sam_gov', fit_score: 90, status: 'new' },
            { id: 'opp-2', title: 'Opportunity B', agency: 'Agency B', source: 'govcon', fit_score: 80, status: 'qualified' },
            { id: 'opp-3', title: 'Opportunity C', agency: 'Agency C', source: 'sam_gov', fit_score: 70, status: 'new' }
          ],
          total: 3,
          page: 1,
          limit: 25
    ...
    // Test sort by fit score descending
    await sortSelect.selectOption('fit_score_desc');
    await page.waitForTimeout(500);
    
    const titles = page.locator('[data-testid="opportunity-title"]');
    const titleTexts = await titles.allInnerTexts();
    expect(titleTexts).toEqual(['Opportunity A', 'Opportunity B', 'Opportunity C']);
    
    // Test sort by fit score ascending
    await sortSelect.selectOption('fit_score_asc');
    await page.waitForTimeout(500);
    
    const sortedTitles = await titles.allInnerTexts();
    expect(sortedTitles).toEqual(['Opportunity C', 'Opportunity B', 'Opportunity A']);
    
    // Test sort by title ascending
    await sortSelect.selectOption('title_asc');
    await page.waitForTimeout(500);
    
    const titleSorted = await titles.allInnerTexts();
    expect(titleSorted).toEqual(['Opportunity A', 'Opportunity B', 'Opportunity C']);
    
    // Test sort by title descending
    await sortSelect.selectOption('title_desc');
    await page.waitForTimeout(500);
    
    const titleDescSorted = await titles.allInnerTexts();
    expect(titleDescSorted).toEqual(['Opportunity C', 'Opportunity B', 'Opportunity A']);
  });
});
