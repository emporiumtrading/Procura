...
export const authRegression = {
  ...
  validateToken: vi.fn(async (token: string) => {
    ...
    try {
      ...
      return {
        valid: false,
        error: e?.message || 'Invalid signature',
        error_code: 'INVALID_SIGNATURE',
      };
    }
  }),
  checkForRoleEscalation: vi.fn(async (token: string) => {
    const auth = state.auth!;
    const user = await auth.verifyToken?.(token);
    const escalation_detected = true;
    auth.logSecurityEvent?.({
    ...
    return { escalation_detected, original_role: 'user', escalated_role: 'admin' };
  }),
  ...
  manageConcurrentSessions: vi.fn(async (userId: string, maxSessions: number) => {
    ...
    if (count >= maxSessions)
      state.auth?.logSessionEvent?.({
        event: 'concurrent_sessions_at_limit',
        user_id: userId,
        active_sessions: count,
        max_sessions: maxSessions,
        timestamp: new Date().toISOString(),
      });
    if (count > maxSessions && sessions?.length)
      state.auth?.terminateSession?.(sessions[0].session_id);
    return { allowed: true, active_sessions: count, max_sessions: maxSessions };
  }),
  preventSessionFixation: vi.fn(async (token: string) => {
    ...
    state.auth?.logSecurityEvent?.({
      event: 'session_fixation_prevented',
      attacker_session: session?.session_id,
      victim_token: token,
      timestamp: new Date().toISOString(),
    });
    return {
  ...
  validateCSRFProtection: vi.fn(async (sessionId: string, csrfToken: string | null) => {
    ...
    if (session?.status === 'logged_out')
      return { valid: false, error: 'Session invalid', error_code: 'SESSION_INVALID' };
  ...
  performSensitiveOperation: vi.fn(
    async (sessionId: string, _operation: string, _csrfToken: string) => {
  ...
  verifyMFA: vi.fn(async (userId: string, _code: string) => {
    try {
  ...
  checkPasswordExpiration: vi.fn(async (userId: string) => {
    ...
    const lastChanged = info?.password_last_changed
      ? new Date(info.password_last_changed).getTime()
      : 0;
    const daysSince = lastChanged
      ? Math.floor((Date.now() - lastChanged) / (24 * 60 * 60 * 1000))
      : 0;
    state.auth?.requestPasswordReset?.(userId);
    return {
      expired: daysSince >= days,
      days_since_change: daysSince,
      requires_change: daysSince >= days,
    };
  }),
