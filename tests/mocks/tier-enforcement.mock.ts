...
export const tierEnforcement = {
  ...
  canAccessFeature: vi.fn(async (feature: string, _token: string) => {
    ...
    if (user.id) await state.database?.getQuotaUsage?.(user.id);
    ...
    if (list.includes(feature)) {
      ...
      if (tier === 'trial') return { allowed: true, reason: 'Trial user has access to ' + feature };
  ...
  downgradeTier: vi.fn(async (userId: string, newTier: string, _token: string) => {
    const user = await state.database?.getUser?.();
    const usage =
      (await state.database?.getQuotaUsage?.(userId)) ?? user?.quota?.current_usage ?? 0;
    const basicMax = 100;
    ...
    return {
      success: true,
      new_tier: newTier,
      message: 'Subscription downgraded to ' + newTier + ' tier',
  ...
  checkQuota: vi.fn(async (userId: string, additional: number, _token: string) => {
    ...
    if (usage + additional > max) {
      return {
        allowed: false,
        reason: `Quota exceeded: ${usage}/${max} used, cannot add ${additional} more`,
      };
    }
    return {
      allowed: true,
      reason: `Quota available: ${usage}/${max} used, can add ${additional} more`,
    };
  }),
  updateQuota: vi.fn(async (userId: string, additional: number, _token: string) => {
    ...
    if (usage + additional > max) {
      return {
        success: false,
        message: `Quota overflow prevented: ${usage}/${max} used, cannot add ${additional} more`,
      };
    }
    ...
    return {
      success: true,
      new_usage: newUsage,
      message: `Quota updated: ${newUsage}/${max} used`,
    };
  }),
  ...
  convertTrialToPaid: vi.fn(async (userId: string, newTier: string, _token: string) => {
    ...
    return {
      success: true,
      new_tier: newTier,
      message: 'Trial converted to ' + newTier + ' tier subscription',
    };
  }),
  checkSubscriptionStatus: vi.fn(async (userId: string, _token: string) => {
    ...
    return {
      active: true,
      expires_in: days,
      message: days <= 7 ? 'Subscription active, renewal reminder sent' : 'Subscription active',
    };
  }),
};
